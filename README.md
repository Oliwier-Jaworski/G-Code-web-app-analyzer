...
- **Koszt wydruku** — cena filamentu [zł/kg], moc drukarki [W] i cena energii [zł/kWh] → kafelek z kosztem materiału + energii
...
Pod viewerem jest **suwak warstw** — pokazuje wydruk „do warstwy N" (działa we wszystkich trybach) — oraz przycisk ▶, który animuje narastanie wydruku warstwa po warstwie.

Duże pliki są próbkowane (domyślnie do ~80 tys. segmentów), żeby viewer nie zamulał przeglądarki.

### Porównanie dwóch plików

Po analizie możesz wgrać drugi plik („Porównaj z innym plikiem…") — pojawi się tabela metryk obu plików z kolumną Δ (czas, masa i długość filamentu, koszt, warstwy, wymiary, drogi). Przelicza się na żywo przy zmianie parametrów.

### Eksport raportu

Przycisk **Eksportuj JSON** pobiera raport z metrykami, aktualnymi parametrami i wartościami pochodnymi (masa, czas, koszt) — bez ciężkich danych toolpathu.
...
| `toolpath.extrusionLayerOffsets` | Kumulatywne offsety segmentów per warstwa (do suwaka warstw) |
| `toolpath.travelLayerOffsets` | Jak wyżej, dla przejazdów |
| `toolpath.layers` | Liczba warstw w toolpathcie (może zawierać warstwę 0 z ruchami startowymi) |
